...
function removeSelected() {
    ...
    // ── MULTI-SELECT: hapus semua objek yang dipilih sekaligus ──────────────
    if (obj.type === 'activeSelection') {
        var selectedObjs = obj.getObjects().slice(); // salin array sebelum loop
        var hasTable = selectedObjs.some(function (o) {
            return o._isTable || (o.name && pg.tableStore[o.name]);
        });

        if (hasTable) {
            Swal.fire({
                title: 'Hapus ' + selectedObjs.length + ' Elemen?',
                text: 'Termasuk tabel yang dipilih.',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: '<i class="bi bi-trash"></i> Hapus Semua',
                ...
                selectedObjs.forEach(function (o) {
                    if (o.name && pg.tableStore[o.name]) delete pg.tableStore[o.name];
                    canvas.remove(o);
                });
                canvas.renderAll();
                saveState();
                Swal.fire({ toast: true, position: 'bottom-end', icon: 'success',
                    title: selectedObjs.length + ' elemen dihapus',
                    showConfirmButton: false, timer: 1500 });
            });
        } else {
            canvas.discardActiveObject();
            selectedObjs.forEach(function (o) { canvas.remove(o); });
            canvas.renderAll();
            saveState();
        }
        return;
    ...
    // ── SINGLE: Hapus kop surat — minta konfirmasi ──────────────────────────
    ...
    // ── SINGLE: Hapus tabel — minta konfirmasi ──────────────────────────────
    ...
    // ── SINGLE: Hapus elemen biasa — langsung ──────────────────────────────
    canvas.remove(obj);
    canvas.discardActiveObject();
    canvas.renderAll();
    saveState();
