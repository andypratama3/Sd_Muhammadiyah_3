...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form action="{{ route('dashboard.datasekolah.achivement.update', $achivement->slug) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="hidden" name="slug" value="{{ $achivement->slug }}">
            <div class="form-group mt-2">
                <label for="name">Nama</label>
                <input type="text" class="form-control" name="name" id="name" value="{{ old('name', $achivement->name) }}" placeholder="Masukan Nama">
            </div>
            <div class="form-group mt-2 mb-2">
                <label for="">Order</label>
                <input type="number" class="form-control" name="order" id="order" value="{{ old('order', $achivement->order) }}" placeholder="Masukan Posisi">
            </div>
            <div class="form-group mt-4">
                ...
                <div class="mt-3 text-center">
                    ...
                    <img src="{{ asset('storage/img/achivement/'.$achivement->foto) }}" id="output" alt="" style="border-radius: 10px;" class="img-fluid">
                </div>
            </div>

            <div class="form-group mt-2">
                <a  href="{{ route('dashboard.datasekolah.achivement.index') }}" class="btn btn-danger btn-sm">Kembali</a>
