...
<div class="card mb-4">
    ...
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold text-primary">Edit Kerja Sama</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('dashboard.datasekolah.cooperation.update', $cooperation->slug) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="hidden" name="slug" value="{{ $cooperation->slug }}">
            <div class="form-group mt-2">
                <label for="name">Nama</label>
                <input type="text" class="form-control" name="name" id="name" value="{{ old('name', $cooperation->name) }}" placeholder="Masukan Nama">
            </div>
            <div class="form-group mt-2 mb-2">
                <label for="">Order</label>
                <input type="number" class="form-control" name="order" id="order" value="{{ old('order', $cooperation->order) }}" placeholder="Masukan Posisi">
            </div>
            <div class="form-group mt-4">
                ...
                <div class="mt-3 text-center">
                    ...
                    <img src="{{ asset('storage/img/cooperation/'.$cooperation->foto) }}" id="output" alt="" style="border-radius: 10px;" class="img-fluid">
                </div>
            </div>
