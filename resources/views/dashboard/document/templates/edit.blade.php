...
<div class="editor-topbar">
    ...
    <form action="{{ route('dashboard.documents.templates.update', $template->id) }}"
...
{{-- ── SIDEBAR KIRI (identik dengan create) ── --}}
<div id="sidebarLeft">

    <div class="sb-section">
        <div class="sb-section-head"><i class="bi bi-grid-1x2"></i> Komponen</div>
        <div class="sb-section-body">
            <button type="button" class="sb-btn primary" data-bs-toggle="modal" data-bs-target="#modalKop">
                <i class="bi bi-bank"></i> Kop Surat Sekolah
            </button>
            <button type="button" class="sb-btn outline-primary" data-bs-toggle="modal" data-bs-target="#modalVariable">
                <i class="bi bi-braces"></i> Tambah Variabel
            </button>
            <button type="button" class="sb-btn" onclick="addText()">
                <i class="bi bi-fonts"></i> Tambah Teks
            </button>
            <button type="button" class="sb-btn outline-success" data-bs-toggle="modal" data-bs-target="#modalTable">
    ...
    <div class="sb-section">
        <div class="sb-section-head"><i class="bi bi-shapes"></i> Shapes</div>
        <div class="sb-section-body">
            <div class="sb-row">
                <button class="sb-btn" onclick="addShape('rect')"><i class="bi bi-square"></i> Kotak</button>
                <button class="sb-btn" onclick="addShape('circle')"><i class="bi bi-circle"></i> Ellipse</button>
            </div>
            <div class="sb-row">
                <button class="sb-btn" onclick="addShape('triangle')"><i class="bi bi-triangle"></i> Segitiga</button>
                <button class="sb-btn" onclick="addShape('hline')"><i class="bi bi-dash-lg"></i> Garis</button>
            </div>
        </div>
    </div>
    ...
    <div class="sb-section" id="formatToolbar">
        ...
        <div class="sb-section-body">
            ...
            <div>
                ...
                <input type="color" id="fontColor" class="fmt-color" value="#000000" onchange="applyFormat('fill', this.value)">
            </div>
            <div class="fmt-btn-row">
                <button id="btnBold"     class="fmt-btn" onclick="toggleBold()"         ><strong>B</strong></button>
                <button id="btnItalic"   class="fmt-btn" onclick="toggleItalic()"        ><em>I</em></button>
                <button id="btnUnderline"class="fmt-btn" onclick="toggleUnderline()"     ><u>U</u></button>
                <button id="btnStrike"   class="fmt-btn" onclick="toggleStrikethrough()" ><s>S</s></button>
            </div>
            <div class="fmt-btn-row">
                <button id="btnAlignLeft"    class="fmt-btn" onclick="applyFormat('textAlign','left')"    ><i class="bi bi-text-left"></i></button>
                <button id="btnAlignCenter"  class="fmt-btn" onclick="applyFormat('textAlign','center')"  ><i class="bi bi-text-center"></i></button>
                <button id="btnAlignRight"   class="fmt-btn" onclick="applyFormat('textAlign','right')"   ><i class="bi bi-text-right"></i></button>
                <button id="btnAlignJustify" class="fmt-btn" onclick="applyFormat('textAlign','justify')" ><i class="bi bi-justify"></i></button>
            </div>
            <div>
            ...
            <div class="fmt-grid-2">
                <div><span class="fmt-label">X</span><input type="number" id="objX" class="fmt-input" value="0" onchange="applyPosition('x', parseInt(this.value))"></div>
                <div><span class="fmt-label">Y</span><input type="number" id="objY" class="fmt-input" value="0" onchange="applyPosition('y', parseInt(this.value))"></div>
                <div><span class="fmt-label">Lebar</span><input type="number" id="objWidth" class="fmt-input" value="200" onchange="applyWidth(parseInt(this.value))"></div>
                <div><span class="fmt-label">Tinggi</span><input type="number" id="objHeight" class="fmt-input" value="50" onchange="applyHeight(parseInt(this.value))"></div>
            </div>
            <div>
                <div class="fmt-range-row">
...
<div id="editorRoot">
    <div id="canvasArea">
        <div class="canvas-toolbar">
            <span class="pill-a4"><i class="bi bi-file-earmark"></i> A4 · 210×297mm · 794×1123px</span>
            ...
            <div class="zoom-row">
                ...
                <button class="zoom-btn" onclick="zoomReset()" style="font-size:.7rem;font-weight:700;width:auto;padding:0 6px">1:1</button>
            </div>
            <div class="ct-divider"></div>
            <label class="ct-switch"><input type="checkbox" id="toggleGrid" onchange="toggleGrid(this.checked)"><i class="bi bi-grid-3x3"></i> Grid</label>
            <label class="ct-switch"><input type="checkbox" id="toggleSnap" checked onchange="setSnapEnabled(this.checked)"><i class="bi bi-magnet"></i> Snap</label>
            <label class="ct-switch"><input type="checkbox" id="toggleMargin" onchange="toggleMarginGuides(this.checked)"><i class="bi bi-border-outer"></i> Margin</label>
            <label class="ct-switch"><input type="checkbox" id="toggleRuler" checked onchange="toggleRulerVis(this.checked)"><i class="bi bi-rulers"></i> Ruler</label>
            <div class="ct-divider"></div>
            <div class="coord-bar">
        ...
        <div id="variablePanel">
            <div class="var-panel-wrap">
                <div class="var-panel-head"><i class="bi bi-braces text-primary"></i> Variabel Tersedia <small style="font-weight:400;color:var(--text-muted);margin-left:4px">— klik untuk taruh ke canvas</small></div>
...
<script src="{{ asset('asset_dashboard/js/document/init.js') }}"></script>
<script>
    window.EXISTING_CANVAS_JSON = {!! json_encode($template->canvas_json ?? null) !!};
