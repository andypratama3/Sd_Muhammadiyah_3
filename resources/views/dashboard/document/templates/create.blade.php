...
<style>
     ...
     * ── Ruler layout grid ────────────────────────────────────
     *
     *   col 0 = var(--ruler-sz)  = 22px    ← ruler vertikal & corner
     *   col 1 = minmax(var(--a4-w), 1fr)   ← canvas + ruler horizontal
     *
     *   row 0 = var(--ruler-sz)  = 22px    ← ruler horizontal & corner
     *   row 1 = 1fr                        ← isi canvas
     */
    #rulerLayout {
        display: grid;
    ...
    /* ── Ruler canvases ─────────────────────────────────────── */
    /* Lebar (rulerH) dan tinggi (rulerV) diset oleh JS via drawRulersBase() */
    #rulerH {
        display: block;
     ...
     * ── Page block ───────────────────────────────────────────
     *
     *  Ukuran default (zoom=1):  794px × 1123px  = A4 @ 96dpi
     *  JS (setZoom) akan mengubah width/height saat zoom berubah.
     *
     *  Perbandingan dengan PDF:
     *    794px × 0.75pt/px = 595.5 pt  (A4 standar = 595.28pt)
     *   1123px × 0.75pt/px = 842.25pt  (A4 standar = 841.89pt)
     *   Error < 0.04% — tidak terlihat di print.
     */
        ...
        <form action="{{ route('dashboard.documents.templates.store') }}" method="POST" id="templateForm">
            ...
            <div class="card mb-2 shadow-sm">
                <div class="card-body py-1 px-2 d-flex align-items-center flex-wrap gap-2">
                    ...
                    {{-- Koordinat live (px + mm ditampilkan JS) --}}
            ...
            {{-- ============================================================
                 ...
                 Struktur grid ruler (nilai semua dalam px):

                 ┌──────────┬──────────────────────────────────────────────┐
                 │  corner  │        horizontal ruler  (0–210 mm)          │
                 │  22×22   │        canvas lebar sesuai zoom              │
                 ├──────────┼──────────────────────────────────────────────┤
                 │ vertical │   .page-block  794×1123px  (A4 @ 96dpi)     │
                 │  ruler   │   bisa multi-page, susun vertikal           │
                 │  22px    │                                              │
                 │ (0–297mm)│                                              │
                 └──────────┴──────────────────────────────────────────────┘

                 Kalkulasi utama (dari JS template-editor.js):
                   CANVAS_W = Math.round(210 * 96/25.4) = 794 px
                   CANVAS_H = Math.round(297 * 96/25.4) = 1123 px
                   MARGIN   = Math.round(20  * 96/25.4) = 76 px
                   PX_TO_PT = 72/96 = 0.75
            ================================================================= --}}
            <div id="editorContainer">
                <div id="rulerLayout">
                    ...
                    {{-- horizontal ruler: sumbu X, 0–210 mm
                         - Lebar diset JS: rulerH.width = round(CANVAS_W × zoom)
                         - Setiap tick = 1mm, label setiap 10mm
                         - Bubble tooltip mm saat hover --}}
                    ...
                    {{-- vertical ruler: sumbu Y, 0–297 mm
                         - Tinggi diset JS: rulerV.height = round(CANVAS_H × zoom)
                         - Teks dirotasi −90° --}}
                    ...
                    {{-- canvas pages: diisi JS (createPage)
                         Setiap .page-block = satu halaman A4
                         CSS default width/height = var(--a4-w) × var(--a4-h)
                         JS setZoom() mengubah width/height sesuai currentZoom --}}
