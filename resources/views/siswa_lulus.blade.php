...
<div class="row mx-2">
    ...
    <div class="col-md-12">
        <div class="row justify-content-start g-4 px-3">
            @foreach($siswas as $siswa)
                <div class="col-lg-3 col-md-4 col-sm-6 col-6 student-card-wrapper">
                    <div class="book mx-auto">
                        ...
                        <div class="cover">
                            <img src="https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=2940&auto=format&fit=crop"
                                 alt="Foto Siswa">
                            <p>{{ $siswa['name'] }}</p>
                        </div>
                        ...
                        <div class="card-content">
                            <h6>{{ $siswa['nama'] }}</h6>
                            <p>NISN: {{ $siswa['nisn'] ?? '-' }}</p>
                            <p>Tahun: {{ $siswa['tahun_lulus'] ?? '2019' }}</p>
                            <a href="{{ asset('storage/img/siswa/' . $siswa['foto'] ?? '') }}" class="btn btn-primary btn-sm mt-2">Download Foto</a>
                        </div>
                    </div>
                </div>
            @endforeach
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        books.forEach(book => {
            ...
            // Toggle rotate class on click (mobile behavior)
            book.addEventListener('click', function (e) {
                if (window.innerWidth <= 767) {
                    // Tutup semua cover yang terbuka selain yang diklik
                    books.forEach(b => {
                        if (b !== book) {
                    ...
                    // Toggle class
                    book.classList.toggle('active-mobile');
                }
            });
        });

        // Tutup saat klik di luar elemen book
        document.addEventListener('click', function (e) {
            if (window.innerWidth <= 767) {
                if (!e.target.closest('.book')) {
                    books.forEach(b => b.classList.remove('active-mobile'));
                }
            }
        });
    });
